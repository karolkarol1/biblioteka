...
class phpinvoice extends FPDF_rotation  {
    ...
    ******************************************/
	...
	public function addItem($item,$quantity,$price,$total) {
		$p['item'] 			= $item;
	...
	public function Header() {
	    ...
	    $this->Cell(0,5,iconv("UTF-8", "ISO-8859-16",strtoupper($this->title)),0,1,'R');
		...
		if(!empty($this->reference)) {
		  ...
		  $this->Cell(32,$lineheight,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['number']).':'),0,0,'L');
		...
		$this->SetFont($this->font,'B',9);
		$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
		$this->Cell(32,$lineheight,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['date'])).':',0,0,'L');	
		$this->SetTextColor(50,50,50);
		$this->SetFont($this->font,'',9);
		...
		if(!empty($this->time)){
		  ...
		  $this->SetFont($this->font,'B',9);
		  $this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
		  $this->Cell(32,$lineheight,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['time'])).':',0,0,'L');	
		  $this->SetTextColor(50,50,50);
		  $this->SetFont($this->font,'',9);
		...
		if(!empty($this->due)){
			...
			$this->SetFont($this->font,'B',9);
			$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
			$this->Cell(32,$lineheight,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['due'])).':',0,0,'L');	
			$this->SetTextColor(50,50,50);
			$this->SetFont($this->font,'',9);
		...
		if($this->PageNo()== 1) {
			...
			if($this->display_tofrom === true) {
				...
				for($i=1; $i<max(count($this->from),count($this->to)); $i++) {
					$this->Cell($width,$lineheight,iconv("UTF-8", "ISO-8859-16",$this->from[$i]),0,0,'L');
					$this->Cell(0,$lineheight,iconv("UTF-8", "ISO-8859-16",$this->to[$i]),0,0,'L');
					$this->Ln(5);
				}	
				$this->Ln(-6);
				$this->Ln(5);
		...
		if(!isset($this->productsEnded))  {
			...
			$this->Cell($this->firstColumnWidth,10,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['product'])),0,0,'L',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['qty'])),0,0,'C',0);
			/*if(isset($this->vatField)) {
				$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
				$this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['vat'])),0,0,'C',0);
			}*/
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['price'])),0,0,'C',0);
			if(isset($this->discountField)) {
				$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
				$this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['discount'])),0,0,'C',0);
			}
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-16",strtoupper($this->lang['total'])),0,0,'C',0);
			$this->Ln();
	...
	public function Body() {	
		...
		if($this->items) {
			foreach($this->items as $item) 
			{
				if($item['description']) 
				{
					...
					$calculateHeight->MultiCell($this->firstColumnWidth,3,iconv("UTF-8", "ISO-8859-16",$item['description']),0,'L',1);
				...
				$x = $this->GetX();
				$this->Cell($this->firstColumnWidth,$cHeight,iconv("UTF-8", "ISO-8859-16",$item['item']),0,0,'L',1);
				if($item['description']) {
					$resetX = $this->GetX();
					$resetY = $this->GetY();
					...
					$this->MultiCell($this->firstColumnWidth,3,iconv("UTF-8", "ISO-8859-16",$item['description']),0,'L',1);
					//Calculate Height
					$newY = $this->GetY();
		...
		foreach($this->addText as $text)  {
			if($text[0] == 'title') 
			{
				$this->SetFont($this->font,'b',9);
				$this->SetTextColor(50,50,50);
				$this->Cell(0,10,iconv("UTF-8", "ISO-8859-16",strtoupper($text[1])),0,0,'L',0);
				$this->Ln();
				$this->SetLineWidth(0.3);
				$this->SetDrawColor($this->color[0],$this->color[1],$this->color[2]);
				$this->Line($this->margins['l'], $this->GetY(),$this->document['w']-$this->margins['r'], $this->GetY());
			...
			if($text[0] == 'paragraph') 
			{
				...
				$this->SetFont($this->font,'',8);
				$this->MultiCell(0,4,iconv("UTF-8", "ISO-8859-16",$text[1]),0,'L',0);
				$this->Ln(4);
			}
		}
	}
